...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadUserData() {
    try {
      ...
      if (response && !response.error) {
        ...
        throw new Error(
          response?.error || "Не вдалося завантажити дані користувача"
        );
      }
    } catch (error) {
  ...
  if (avatarUpload) {
    avatarUpload.addEventListener("change", async (e) => {
      ...
      if (saveBtn) {
        ...
        saveBtn.innerHTML =
          '<i class="fas fa-spinner fa-spin"></i> Завантаження аватара...';
      }
      ...
      try {
        ...
        if (file.size > MAX_FILE_SIZE) {
          throw new Error(
            "Розмір файлу перевищує 200KB. Будь ласка, виберіть менше зображення."
          );
        }
        ...
        console.log(
          "Розмір даних:",
          ((optimizedImage.length * 3) / 4).toFixed(2) + " bytes"
        );
  ...
  if (saveBtn) {
    saveBtn.addEventListener("click", async (e) => {
      ...
      saveBtn.disabled = true;
      saveBtn.innerHTML =
        '<i class="fas fa-spinner fa-spin"></i> Зберігання...';
      ...
      try {
        ...
        if (isPasswordFormVisible) {
          ...
          if (newPassword.length < 6) {
            throw new Error(
              "Новий пароль повинен містити принаймні 6 символів"
            );
          }
        ...
        showNotification(
          `Помилка: ${
            error.message || "Не вдалося зберегти зміниSave changes"
          }`,
          "error"
        );
      } finally {
